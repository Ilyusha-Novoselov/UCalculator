...
std::string UCalculator::PControl::DoEditorCommand(int command)
{
    _state = cEditing;
...
void UCalculator::PControl::DoProcessorCommand(int command)
{
    std::string edit;
...
uint8_t UCalculator::PControl::DoMemoryCommand(int command)
{
    PNumber number(_editor->getNumber(), _p);
...
void UCalculator::PControl::DoReset(bool flag)
{
    _editor->Clear();
    _processor->reset();
    if(flag)
